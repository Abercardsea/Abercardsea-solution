...
def weather_factor(forecast: pd.DataFrame) -> pd.Series:
    ...
    """
    weather = forecast["W"] # weather type new code.
    weather_list = []
    no_change = [0, 1]
    med_change = [2, 3, 5, 8]
...
def get_solar_prediction(forecast: pd.DataFrame) -> pd.DataFrame:
    ...
    next 24 hours.
    """
    #forecast = interp_30min(forecast) no longer necessary.
...
def save_to_db_solar(solar):
    """Save solar data to database."""
    engine = create_engine("sqlite:///database/llanwrydd.db")
    solar.to_sql('solarPrediction', con=engine, if_exists='append', index=False)